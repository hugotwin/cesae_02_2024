package Exercicio3.Ingredientes;

import Exercicio3.Enum.Origem;
import Exercicio3.Enum.TipoQueijo;
import Exercicio3.Enum.UnidadeMedida;

public class Queijo extends Topping{

    private TipoQueijo tipo;


    public Queijo(int codigo, String nome, UnidadeMedida unidade_medida, Double numero_calorias_unidade_medida, Origem origem, TipoQueijo tipo) {
        super(codigo, nome, unidade_medida, numero_calorias_unidade_medida, origem);

        this.tipo=tipo;
    }
}
